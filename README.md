...
- 🎯 Entry points to execute your program using `python -m <flask_template_merito>` or `$ flask_template_merito` with basic CLI argument parsing.
...
---
# flask_template_merito Flask Application

Awesome flask_template_merito created by Remediii
...
```bash
git clone https://github.com/Remediii/flask-template-merito flask_template_merito
cd flask_template_merito
make install
```
...
```bash
pip install flask_template_merito
```
...
```bash
$ flask_template_merito
```

or

```bash
$ python -m flask_template_merito
```
...
```bash
flask_template_merito create-db   # run once
flask_template_merito populate-db  # run once (optional)
flask_template_merito add-user -u admin -p 1234  # ads a user
flask_template_merito run
```
